...
import { Phone, MessageCircle, AlertTriangle, ShieldCheck, Truck, ChevronRight, X, BookMarked, CheckCircle } from 'lucide-react';
...
export default function HomePage() {
  ...
  const [modalStage, setModalStage] = useState('LEAD_CAPTURE');
  ...
  const toggleCatalog = (product) => {
    ...
    let updated;
    if (exists) {
      updated = catalog.filter(p => p.id !== product.id);
  ...
  const isInCatalog = (id) => catalog.some(p => p.id === id);
      ...
      <section className="hero">
        <div className="container hero-content" style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="hero-actions">
            <button className="btn-primary" onClick={() => window.scrollTo({ top: document.getElementById('products').offsetTop - 80, behavior: 'smooth' })}>
              Explore Products
            </button>
              ...
              <div key={product.id} className="product-card">
                ...
                <div className="product-info">
                  ...
                  <div className="product-actions">
                    ...
                    <button
                      className={`btn-catalog mt-4 ${isInCatalog(product.id) ? 'in-catalog' : ''}`}
                      onClick={() => toggleCatalog(product)}
                      title={isInCatalog(product.id) ? 'Remove from Catalog' : 'Add to Catalog'}
                    >
                      <BookMarked size={16} />
                      {isInCatalog(product.id) ? 'Saved' : 'Save'}
                    </button>
                  </div>
                </div>
              </div>
